...
/* =========================================================
   AQUORAX OS — CINEMATIC INTERFACE OVERHAUL v1
   Adds a new presentation shell without replacing stored data/systems.
   ========================================================= */
(function(){
  if(window.__aqxOsCinematicOverhaulV1) return;
  window.__aqxOsCinematicOverhaulV1 = true;
  function $(id){return document.getElementById(id);} 
  function esc(v){return String(v==null?'':v).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c];});}
  function activeTankName(){try{return (typeof currentTank==='function' && currentTank() && currentTank().name) || localStorage.getItem('aquoraxActiveTankName') || 'My Reef';}catch(e){return 'My Reef';}}
  function latestParams(){try{return typeof latestMergedParameters==='function'?latestMergedParameters():{latest:null,values:{}};}catch(e){return {latest:null,values:{}};}}
  function score(){try{return typeof calculateAqxScore==='function'?calculateAqxScore():{score:null,status:'No Data'};}catch(e){return {score:null,status:'No Data'};}}
  function latestIcp(){
    try{
      var fn=window.getTankScopedArray||window.aqxGetTankScopedArray;
      var arr= typeof fn==='function'?fn('icpTests'):(JSON.parse(localStorage.getItem('aquoraxIcpTests')||'[]'));
      return (arr||[]).slice().sort(function(a,b){return new Date(b.createdIso||b.date||0)-new Date(a.createdIso||a.date||0);})[0]||null;
    }catch(e){return null;}
  }
  function statusClass(status,hasData){
    if(!hasData) return 'nodata';
    status=String(status||'').toLowerCase();
    if(/unstable|urgent|bad|danger|high/.test(status)) return 'bad';
    if(/watch|review|low|warn/.test(status)) return 'watch';
    return 'good';
  }
  function paramSignal(key,label,unit){
    var d=latestParams(), v=d.values?d.values[key]:undefined;
    if(v===undefined||v===null||v==='') return {label:label,value:'No data',cls:'nodata'};
    return {label:label,value:String(d.values[key+'__raw']||v)+(unit?' '+unit:''),cls:'good'};
  }
  function buildHomeShell(){
    var home=$('home'); if(!home || $('aqxOsHomeShell')) return;
    var shell=document.createElement('div'); shell.id='aqxOsHomeShell'; shell.className='aqxOsHomeShell';
    shell.innerHTML='\
      <section class="aqxOsHero">\
        <div class="aqxOsHeroTop">\
          <div class="aqxOsMark"><img src="aquorax-logo.png" alt="AquoraX"><div><span>AquoraX OS</span><strong id="aqxOsTankName">My Reef</strong></div></div>\
          <div class="aqxOsLivePill" id="aqxOsLivePill">Live tank</div>\
        </div>\
        <div class="aqxOsHeroMain">\
          <div class="aqxOsOrb"><div><b id="aqxOsMainScore">—</b><span id="aqxOsMainScoreLabel">No Data</span></div></div>\
          <h1 id="aqxOsHeroTitle">Your reef, simplified.</h1>\
          <p id="aqxOsHeroCopy">AquoraX turns water tests, ICP and dosing into one calm reef state — not another dashboard of boxes.</p>\
        </div>\
        <div class="aqxOsHeroActions">\
          <button class="primaryBtn" onclick="openPage(\'log\')">Log water test</button>\
          <button class="secondaryBtn" onclick="openPage(\'dosing\')">Open Reef Intelligence</button>\
        </div>\
        <div class="aqxOsSignalStrip">\
          <div id="aqxOsSignalAlk" class="aqxOsSignal nodata"><span>Alkalinity</span><strong>No data</strong></div>\
          <div id="aqxOsSignalIcp" class="aqxOsSignal nodata"><span>ICP</span><strong>No scan</strong></div>\
          <div id="aqxOsSignalNext" class="aqxOsSignal nodata"><span>Next Action</span><strong>Start tracking</strong></div>\
        </div>\
      </section>\
      <div class="aqxOsSectionTitle"><div><span>Reef Command</span><h2>Control without clutter</h2></div></div>\
      <div class="aqxOsDockGrid">\
        <div class="aqxOsDockCard" onclick="openPage(\'dosing\')"><span>🧬</span><strong>Dosing & ICP</strong><small>Brand ecosystem, reef health and verified ICP intelligence.</small></div>\
        <div class="aqxOsDockCard" onclick="openPage(\'jobs\')"><span>⏱️</span><strong>Reef Rhythm</strong><small>Maintenance, dosing and care tasks in one routine.</small></div>\
        <div class="aqxOsDockCard" onclick="openPage(\'growth\')"><span>🪸</span><strong>Livestock</strong><small>Coral and livestock growth without spreadsheet clutter.</small></div>\
        <div class="aqxOsDockCard" onclick="openPage(\'graphs\')"><span>📈</span><strong>Stability Trends</strong><small>See drift and long-term chemistry patterns.</small></div>\
      </div>';
    var cloud=$('aqxCloudHomeBtn');
    if(cloud && cloud.nextSibling) home.insertBefore(shell, cloud.nextSibling); else home.insertBefore(shell, home.firstChild);
  }
  function updateHomeShell(){
    buildHomeShell();
    var s=score(), d=latestParams(), icp=latestIcp();
    var hasScore=s && s.score!==null && s.score!==undefined;
    var cls=statusClass(s.status,hasScore);
    var scoreEl=$('aqxOsMainScore'), label=$('aqxOsMainScoreLabel');
    if(scoreEl) scoreEl.textContent=hasScore?String(s.score):'—';
    if(label) label.textContent=hasScore?(s.status||'Tracking'):'No Data';
    var orb=document.querySelector('.aqxOsOrb'); if(orb){orb.className='aqxOsOrb '+cls;}
    var tank=$('aqxOsTankName'); if(tank)tank.textContent=activeTankName();
    var title=$('aqxOsHeroTitle'), copy=$('aqxOsHeroCopy'), next='Start tracking';
    if(!hasScore){ if(title)title.textContent='Your reef, ready to wake up.'; if(copy)copy.textContent='Log a test or import ICP to activate AquoraX OS reef state. Until then, the app stays honest: no fake healthy status.'; }
    else if(cls==='good'){ if(title)title.textContent='Your reef looks steady.'; if(copy)copy.textContent='Saved data suggests a calm reef state. Keep testing consistently and let AquoraX watch for drift.'; next='Keep rhythm'; }
    else if(cls==='watch'){ if(title)title.textContent='AquoraX sees something to review.'; if(copy)copy.textContent='No panic. Check the latest outlier, confirm the result, then correct slowly using manufacturer guidance.'; next='Review trend'; }
    else { if(title)title.textContent='Review before changing anything.'; if(copy)copy.textContent='AquoraX is flagging instability or uncertain data. Confirm values first — no guesswork, no fake drama.'; next='Review now'; }
    var alk=paramSignal('alkalinity','Alkalinity','dKH'), alkEl=$('aqxOsSignalAlk');
    if(alkEl){alkEl.className='aqxOsSignal '+alk.cls; alkEl.innerHTML='<span>Alkalinity</span><strong>'+esc(alk.value)+'</strong>';}
    var icpEl=$('aqxOsSignalIcp');
    if(icpEl){
      var val=icp?((icp.lab||'ICP')+' · '+(icp.date||'saved')):'No scan';
      icpEl.className='aqxOsSignal '+(icp?'good':'nodata');
      icpEl.innerHTML='<span>ICP</span><strong>'+esc(val)+'</strong>';
    }
    var nextEl=$('aqxOsSignalNext'); if(nextEl){nextEl.className='aqxOsSignal '+(hasScore?cls:'nodata'); nextEl.innerHTML='<span>Next Action</span><strong>'+esc(next)+'</strong>';}
  }
  function enhanceDosingPage(){
    var page=$('dosing'); if(!page || page.dataset.aqxOsEnhanced) return;
    page.dataset.aqxOsEnhanced='1';
    var hero=$('aqxBrandHero');
    if(hero){
      var badge=document.createElement('div'); badge.className='aqxOsSectionTitle aqxDosingOsIntro';
      badge.innerHTML='<div><span>AquoraX OS</span><h2>Reef ecosystem intelligence</h2></div>';
      hero.parentNode.insertBefore(badge, hero);
    }
  }
  function refresh(){try{updateHomeShell(); enhanceDosingPage();}catch(e){console.warn('AquoraX OS cinematic refresh failed',e);}}
  document.addEventListener('DOMContentLoaded',function(){setTimeout(refresh,120);setTimeout(refresh,650);});
  var oldHome=window.renderHomeDashboard;
  if(typeof oldHome==='function' && !oldHome.__aqxOsCinematicWrapped){
    window.renderHomeDashboard=function(){var out=oldHome.apply(this,arguments); setTimeout(refresh,0); return out;};
    window.renderHomeDashboard.__aqxOsCinematicWrapped=true;
  }
  var oldOpen=window.openPage;
  if(typeof oldOpen==='function' && !oldOpen.__aqxOsCinematicWrapped){
    window.openPage=function(page){var out=oldOpen.apply(this,arguments); document.body.setAttribute('data-aqx-page',page||'home'); setTimeout(refresh,60); return out;};
    window.openPage.__aqxOsCinematicWrapped=true;
  }
  ['saveIcpTest','addDosingProduct','logDose','deleteDosingProduct','saveParameterTest','saveTestResult'].forEach(function(name){
    var fn=window[name];
    if(typeof fn==='function' && !fn.__aqxOsCinematicWrapped){
      window[name]=function(){var out=fn.apply(this,arguments); setTimeout(refresh,120); return out;};
      window[name].__aqxOsCinematicWrapped=true;
    }
  });
})();
