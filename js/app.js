...
/* === AquoraX Reef Intelligence Engine v1 ===
   Adds ICP confidence, reef health scoring, ecosystem-aware insights and prediction cards
   without replacing existing dosing/notification/storage systems. */
(function(){
  function $(id){return document.getElementById(id);} 
  function esc(v){return String(v==null?'':v).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c];});}
  ...
  function reefHealth(){var test=latestIcp(), products=dosingProducts(), hist=dosingHistory(), signals=getSignals(test); var score=100; var notes=[]; var age=icpAgeDays(test);
    if(!test){score-=28; notes.push('No ICP result saved yet');}
    else if(age!==null && age>45){score-=12; notes.push('ICP is '+age+' days old');}
    signals.forEach(function(s){score-=s.status==='high'?10:7;});
    if(signals.length) notes.push(signals.length+' ICP item'+(signals.length===1?'':'s')+' need review');
    if(!products.length){score-=18; notes.push('No active ecosystem products');}
    var low=products.filter(function(p){var l=daysLeft(p);return l!==null&&l<=n(p.lowStockDays,14);});
    if(low.length){score-=Math.min(18,low.length*6); notes.push(low.length+' product'+(low.length===1?'':'s')+' near refill');}
    if(!hist.length){score-=8; notes.push('No dose/check history');}
    score=Math.max(0,Math.min(100,Math.round(score)));
    var cls=score>=82?'good':score>=62?'warn':'bad';
    var label=score>=82?'Stable':score>=62?'Watch':'Needs Attention';
    return {score:score,cls:cls,label:label,notes:notes,test:test,signals:signals,age:age,low:low,products:products,hist:hist};
  }
  ...
  function renderReefIntelligence(){var card=ensurePanel(); if(!card)return; var h=reefHealth(), b=currentBrand(); var signalHtml=h.signals.length?h.signals.slice(0,6).map(function(s){return '<span class="'+esc(s.cls)+'">'+esc(s.name)+': '+esc(s.value)+' '+esc(s.unit||'')+' · '+esc(s.status.toUpperCase())+'</span>';}).join(''):'<span class="good">No low/high ICP markers saved</span>';
    var prediction=[];
    if(!h.test) prediction.push({cls:'warn',title:'ICP import needed',text:'Upload or paste an ICP report to unlock real reef chemistry intelligence.'});
    ...
    if(h.low.length) prediction.push({cls:h.low.some(function(p){return daysLeft(p)<=0;})?'bad':'warn',title:'Refill prediction',text:h.low.slice(0,3).map(function(p){return p.name+' in '+daysLeft(p)+' days';}).join(', ')});
    if(h.signals.some(function(s){return /potassium|iodine|iron|strontium/i.test(s.name)})) prediction.push({cls:'warn',title:'Trace trend watch',text:'Trace markers are outside target. Track the next ICP to confirm if this is consumption or a one-off swing.'});
    ...
    card.innerHTML='<div class="aqxReefIntelTop"><div><span class="aqxMiniKicker">AquoraX Reef Intelligence</span><h2>Reef Health Engine</h2><p>'+esc(brandCopy(b,h))+'</p></div><div class="aqxReefScore '+esc(h.cls)+'"><b>'+h.score+'</b><span>'+esc(h.label)+'</span></div></div><div class="aqxReefIntelGrid"><div class="aqxReefIntelBlock"><strong>ICP Signals</strong><div class="aqxReefSignalChips">'+signalHtml+'</div><small>'+(h.test?esc((h.test.lab||'ICP')+' · '+(h.test.date||'latest')):'No ICP test saved')+'</small></div><div class="aqxReefIntelBlock"><strong>Predictions</strong>'+prediction.map(function(p){return '<div class="aqxPrediction '+esc(p.cls)+'"><b>'+esc(p.title)+'</b><span>'+esc(p.text)+'</span></div>';}).join('')+'</div></div><div class="aqxReefIntelFooter"><span>'+esc(b.short)+' ecosystem</span><span>'+h.products.length+' active product'+(h.products.length===1?'':'s')+'</span><span>'+h.hist.length+' dose/check log'+(h.hist.length===1?'':'s')+'</span></div>';
  }
  ...
  if(oldParse && !oldParse.__aqxReefIntelWrapped){
    aqxParseIcpText=function(){var out=oldParse.apply(this,arguments); enhanceProviderNotes(); setTimeout(renderReefIntelligence,120); return out;};
    aqxParseIcpText.__aqxReefIntelWrapped=true;
  }
