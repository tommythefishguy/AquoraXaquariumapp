...
(function(){
  ...
  window.aqxConfirmIcpReview=function(){var test=latestIcp();if(!test)return;var m=$('aqxIcpReviewModal');var rows=Array.from((m||document).querySelectorAll('.aqxIcpReviewRow')).map(function(r){return {name:(r.querySelector('.rvName')||{}).value||'',value:(r.querySelector('.rvValue')||{}).value||'',unit:(r.querySelector('.rvUnit')||{}).value||'',status:(r.querySelector('.rvStatus')||{}).value||'ok',excludedFromReefHealth:!!(r.querySelector('.rvExclude')||{}).checked,reviewed:true};}).filter(function(p){return p.name||p.value;});test.parameters=rows;test.reviewedByAquoraX=true;test.aqxIcpReviewed=true;test.icpConfidence='reviewed';test.reviewedIso=new Date().toISOString();var cleanNote='Reviewed in AquoraX OS. Excluded rows are ignored by Reef Health.';test.notes=String(test.notes||'').replace(/Low confidence scan detected\. Review ICP before saving\. Reef Health scoring will stay cautious until values are confirmed\.?/ig,'').replace(/Review ICP before saving\.?/ig,'').trim();test.notes=(test.notes?test.notes+'\n':'')+cleanNote;try{var all=(typeof getIcpTests==='function'?getIcpTests():[])||[];if(all.length){all[0]=test;localStorage.setItem(tankKey('icpTests'),JSON.stringify(all));}}catch(e){}aqxCloseIcpReview();setTimeout(renderReefIntelligence,80);if(typeof renderIcpHistory==='function')renderIcpHistory();};
