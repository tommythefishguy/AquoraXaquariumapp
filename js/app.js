...
// === AquoraX Coral Essentials Product Ecosystem Patch ===
(function(){
  const PRODUCT_BASE='aquoraxDosingProducts';
  const HISTORY_BASE='aquoraxDosingHistory';
  const SHOP_URL='https://finestaquatics.co.uk/search?q=coral+essentials';
  const CE_PRODUCTS=[
    {id:'trace-a',name:'Coral Power Trace A',purpose:'Trace A support',size:50,dose:1,img:'assets/coral-essentials-products/ce-trace-a.png',group:'Trace Elements'},
    {id:'trace-b',name:'Coral Power Trace B',purpose:'Trace B support',size:50,dose:1,img:'assets/coral-essentials-products/ce-trace-b.png',group:'Trace Elements'},
    {id:'trace-c',name:'Coral Power Trace C',purpose:'Trace C support',size:50,dose:1,img:'assets/coral-essentials-products/ce-trace-c.png',group:'Trace Elements'},
    {id:'manganese',name:'Coral Power Manganese',purpose:'Manganese trace support',size:50,dose:1,img:'assets/coral-essentials-products/ce-manganese.png',group:'Individual Trace'},
    {id:'lithium',name:'Coral Power Lithium',purpose:'Lithium trace support',size:50,dose:1,img:'assets/coral-essentials-products/ce-lithium.png',group:'Individual Trace'},
    {id:'selenium',name:'Coral Power Selenium',purpose:'Selenium trace support',size:50,dose:1,img:'assets/coral-essentials-products/ce-selenium.png',group:'Individual Trace'},
    {id:'cobalt',name:'Coral Power Cobalt',purpose:'Cobalt trace support',size:50,dose:1,img:'assets/coral-essentials-products/ce-cobalt.png',group:'Individual Trace'},
    {id:'zinc',name:'Coral Power Zinc',purpose:'Zinc trace support',size:50,dose:1,img:'assets/coral-essentials-products/ce-zinc.png',group:'Individual Trace'},
    {id:'chromium',name:'Coral Power Chromium',purpose:'Chromium trace support',size:50,dose:1,img:'assets/coral-essentials-products/ce-chromium.png',group:'Individual Trace'},
    {id:'nickel',name:'Coral Power Nickel',purpose:'Nickel trace support',size:50,dose:1,img:'assets/coral-essentials-products/ce-nickel.png',group:'Individual Trace'},
    {id:'molybdenum',name:'Coral Power Molybdenum',purpose:'Molybdenum trace support',size:50,dose:1,img:'assets/coral-essentials-products/ce-molybdenum.png',group:'Individual Trace'},
    {id:'barium',name:'Coral Power Barium',purpose:'Barium trace support',size:50,dose:1,img:'assets/coral-essentials-products/ce-barium.png',group:'Individual Trace'},
    {id:'vanadium',name:'Coral Power Vanadium',purpose:'Vanadium trace support',size:50,dose:1,img:'assets/coral-essentials-products/ce-vanadium.png',group:'Individual Trace'},
    {id:'rubidium',name:'Coral Power Rubidium',purpose:'Rubidium trace support',size:50,dose:1,img:'assets/coral-essentials-products/ce-rubidium.png',group:'Individual Trace'},
    {id:'boron',name:'Coral Power Boron',purpose:'Boron trace support',size:50,dose:1,img:'assets/coral-essentials-products/ce-boron.png',group:'Individual Trace'},
    {id:'bromide',name:'Coral Power Bromide',purpose:'Bromide trace support',size:50,dose:1,img:'assets/coral-essentials-products/ce-bromide.png',group:'Individual Trace'},
    {id:'iron',name:'Coral Power Iron',purpose:'Iron trace support',size:50,dose:1,img:'assets/coral-essentials-products/ce-iron.png',group:'Individual Trace'},
    {id:'iodine',name:'Coral Power Iodine',purpose:'Iodine trace support',size:50,dose:1,img:'assets/coral-essentials-products/ce-iodine.png',group:'Individual Trace'},
    {id:'reef-ki3',name:'Coral Power Reef KI3',purpose:'Reef KI3 support',size:50,dose:1,img:'assets/coral-essentials-products/ce-reef-k13.png',group:'Reef Support'},
    {id:'strontium',name:'Coral Power Strontium',purpose:'Strontium support',size:100,dose:1,img:'assets/coral-essentials-products/ce-strontium.png',group:'Reef Support'},
    {id:'potassium',name:'Coral Power Potassium',purpose:'Potassium support',size:50,dose:1,img:'assets/coral-essentials-products/ce-potassium.png',group:'Reef Support'},
    {id:'fluoride',name:'Coral Power Fluoride',purpose:'Fluoride support',size:50,dose:1,img:'assets/coral-essentials-products/ce-fluoride.png',group:'Reef Support'}
  ];
  window.aqxCoralEssentialsProducts=CE_PRODUCTS;
  function esc(v){return String(v==null?'':v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}
  function activeTank(){try{return (window.aqxGetActiveTank&&window.aqxGetActiveTank())||{id:'tank_main',name:'Main Tank',type:'reef'};}catch(e){return {id:'tank_main',name:'Main Tank',type:'reef'};}}
  function scoped(base){try{return window.aqxScopedKey?window.aqxScopedKey(base,activeTank().id):(base+'__'+activeTank().id);}catch(e){return base+'__tank_main';}}
  function read(base){try{const v=JSON.parse(localStorage.getItem(scoped(base))||'[]');return Array.isArray(v)?v:[];}catch(e){return [];}}
  ...
  function metaFor(p){return CE_PRODUCTS.find(x=>x.id===p.ceProductId)||CE_PRODUCTS.find(x=>(x.name||'').toLowerCase()===(p.name||'').toLowerCase())||null;}
  function pct(p){const b=num(p.bottleSize,0);return b>0?Math.max(0,Math.min(100,(liveRemaining(p)/b)*100)):0;}
  function status(p){
    const left=daysLeft(p), low=num(p.lowStockDays,14);
    if(left===null) return {cls:'nodata',txt:'No countdown',short:'No Data'};
    if(left<=0) return {cls:'bad',txt:'Refill now',short:'Urgent'};
    if(left<=low) return {cls:'warn',txt:left+' days left',short:'Low Stock'};
    return {cls:'good',txt:'Est. '+left+' days',short:'On Track'};
  }
  function fillProductForm(id){
    const m=CE_PRODUCTS.find(x=>x.id===id); if(!m) return;
    const set=(id,v)=>{const el=document.getElementById(id); if(el) el.value=v;};
    set('doseProductName',m.name); set('doseProductAmount',m.dose); set('doseProductBottleSize',m.size); set('doseProductRemaining',m.size); set('doseProductUnit','ml'); set('doseProductFrequency','daily'); set('doseProductMode','auto'); set('doseProductNotes',m.purpose);
    window.__aqxSelectedCeProductId=m.id;
    try{toggleCustomDoseInterval();}catch(e){}
    const form=document.getElementById('doseProductName'); if(form) form.scrollIntoView({behavior:'smooth',block:'center'});
  }
  window.aqxPickCoralEssentialsProduct=fillProductForm;
  function renderCeLibrary(){
    const list=document.getElementById('aqxCeProductLibrary');
    const dl=document.getElementById('aqxCeProductNames');
    if(dl) dl.innerHTML=CE_PRODUCTS.map(p=>`<option value="${esc(p.name)}"></option>`).join('');
    if(!list) return;
    const active=read(PRODUCT_BASE).map(p=>(p.ceProductId||'')+'|'+(p.name||'').toLowerCase());
    list.innerHTML=CE_PRODUCTS.map(p=>{
      const added=active.includes(p.id+'|'+p.name.toLowerCase()) || active.some(x=>x.endsWith('|'+p.name.toLowerCase()));
      return `<button class="aqxCeProductTile ${added?'added':''}" onclick="aqxPickCoralEssentialsProduct('${esc(p.id)}')" type="button">
        <span class="aqxCeBottleGlow"><img src="${esc(p.img)}" alt="${esc(p.name)}"></span>
        <strong>${esc(p.name)}</strong>
        <small>${esc(p.group)} · ${esc(p.purpose)}</small>
        <em>${added?'Added to tank':'Add to dosing'}</em>
      </button>`;
    }).join('');
  }
  window.aqxRenderCoralEssentialsLibrary=renderCeLibrary;
  const oldAdd=window.addDosingProduct;
  window.addDosingProduct=function(){
    const name=(document.getElementById('doseProductName')?.value||'').trim();
    const amount=num(document.getElementById('doseProductAmount')?.value,NaN);
    if(!name){alert('Add a product name first.');return;}
    if(!Number.isFinite(amount)||amount<=0){alert('Add a dose amount first.');return;}
    const selected=CE_PRODUCTS.find(x=>x.id===window.__aqxSelectedCeProductId)||CE_PRODUCTS.find(x=>x.name.toLowerCase()===name.toLowerCase());
    const freq=document.getElementById('doseProductFrequency')?.value||'daily';
    const bottle=num(document.getElementById('doseProductBottleSize')?.value, selected?selected.size:0);
    const remaining=num(document.getElementById('doseProductRemaining')?.value, bottle||0);
    const product={
      id:'dose_'+Date.now().toString(36)+'_'+Math.random().toString(36).slice(2,6),
      name, amount,
      ...
      createdIso:new Date().toISOString(),
      lastDosedIso:null,
      ceProductId:selected?selected.id:'',
      ceProductImage:selected?selected.img:'',
      ceProductGroup:selected?selected.group:'',
      cePurpose:selected?selected.purpose:''
    };
    const items=read(PRODUCT_BASE); items.push(product); write(PRODUCT_BASE,items);
    ['doseProductName','doseProductAmount','doseProductInterval','doseProductBottleSize','doseProductRemaining','doseProductNotes'].forEach(id=>{const x=document.getElementById(id);if(x)x.value='';});
    window.__aqxSelectedCeProductId='';
    const f=document.getElementById('doseProductFrequency'); if(f) f.value='daily';
    const m=document.getElementById('doseProductMode'); if(m) m.value='auto';
    try{toggleCustomDoseInterval();}catch(e){}
    renderDosingPage();
  ...
  window.renderDosingProducts=function(){
    const box=document.getElementById('dosingProductList'); if(!box) return;
    const items=read(PRODUCT_BASE);
    if(!items.length){box.innerHTML='<div class="aqxCeEmpty"><strong>No Coral Essentials products active yet</strong><span>Add real Coral Essentials bottles from the product library above to start countdowns and dosing intelligence.</span></div>';return;}
    box.innerHTML=items.map(p=>{
      const m=metaFor(p), img=p.ceProductImage||(m&&m.img)||'';
      const st=status(p), pc=pct(p), left=daysLeft(p), unit=esc(p.unit||'ml'), daily=dosePerDay(p);
      const due=typeof nextDueText==='function'?nextDueText(p):{text:'Ready',cls:''};
      return `<div class="aqxCeDoseProduct ${st.cls}">
        <div class="aqxCeDoseBottle">${img?`<img src="${esc(img)}" alt="${esc(p.name)}">`:`<span>${esc((p.name||'?').slice(0,2).toUpperCase())}</span>`}</div>
        <div class="aqxCeDoseInfo">
          <strong>${esc(p.name)}</strong>
          <span>${esc((m&&m.purpose)||p.notes||'Coral Essentials dosing product')}</span>
          <small>${esc((p.mode||'manual')==='auto'?'Auto countdown':'Manual dosing')} · ${esc(activeTank().name||'Tank')}</small>
          <div class="aqxCeDoseBar"><i style="width:${pc.toFixed(0)}%"></i></div>
        </div>
        <div class="aqxCeDoseMetrics">
          <div class="aqxCeRing" style="--p:${pc.toFixed(0)}"><b>${pc.toFixed(0)}%</b></div>
          <div><strong>${daily.toFixed(daily<10?1:0)} ${unit}/day</strong><span>${esc(due.text||'Ready')}</span></div>
          <em>${esc(st.short)}<small>${esc(st.txt)}</small></em>
        </div>
        <div class="aqxCeDoseActions">
          <button class="primaryBtn" onclick="logDose('${esc(p.id)}')">${(p.mode||'manual')==='auto'?'Log Check':'Log Dose'}</button>
          <button class="secondaryBtn" onclick="aqxRefillDosingProduct('${esc(p.id)}')">Refill</button>
          <button class="dangerBtn" onclick="deleteDosingProduct('${esc(p.id)}')">Remove</button>
        </div>
  ...
  function renderOverview(){
    ...
    const products=read(PRODUCT_BASE), history=read(HISTORY_BASE);
    const low=products.filter(p=>{const l=daysLeft(p);return l!==null && l<=num(p.lowStockDays,14);}).length;
    const due=products.filter(p=>{try{const d=nextDueText(p);return String(d.cls||'').includes('bad')||String(d.text||'').toLowerCase().includes('due');}catch(e){return false;}}).length;
    const avg=products.length?Math.round(products.reduce((s,p)=>s+pct(p),0)/products.length):null;
    const classes=products.length?(low?'warn':'good'):'nodata';
    box.innerHTML=[
      {label:'Dosing Health',val:products.length?(low?'Watch':'Tracking'):'—',sub:products.length?products.length+' active product'+(products.length===1?'':'s'):'No data yet',cls:classes},
      {label:'Consistency',val:history.length?Math.min(100,70+Math.min(30,history.length*3))+'%':'—',sub:history.length?history.length+' logs saved':'0 logs saved',cls:history.length?'good':'nodata'},
      {label:'Products Due',val:String(due),sub:due?'Review today':'None due',cls:due?'warn':(products.length?'good':'nodata')},
      {label:'Low Stock',val:String(low),sub:low?'Refill soon':'No warnings',cls:low?'bad':(products.length?'good':'nodata')},
      {label:'CE Products',val:String(products.filter(p=>p.ceProductId||metaFor(p)).length),sub:'Coral Essentials',cls:products.length?'good':'nodata'}
    ].map(x=>`<div class="aqxCeOverviewStat ${x.cls}"><div class="aqxCeMiniRing"><b>${esc(x.val)}</b></div><strong>${esc(x.label)}</strong><span>${esc(x.sub)}</span></div>`).join('');
  }
  function renderInsights(){
    const box=document.getElementById('dosingInsightList'); if(!box) return;
    const products=read(PRODUCT_BASE), history=read(HISTORY_BASE);
    const insights=[];
    if(!products.length){
      insights.push({cls:'warn',title:'No Coral Essentials data yet',text:'Add the Coral Essentials products this tank actually uses to unlock stock countdowns and dosing intelligence.'});
    }else{
      const lows=products.filter(p=>{const l=daysLeft(p);return l!==null && l<=num(p.lowStockDays,14);}).sort((a,b)=>(daysLeft(a)||0)-(daysLeft(b)||0));
      if(lows.length) insights.push({cls:lows.some(p=>(daysLeft(p)||0)<=0)?'bad':'warn',title:'Coral Essentials refill attention',text:lows.slice(0,3).map(p=>p.name+' ('+(daysLeft(p)||0)+' days)').join(', ')});
      if(history.length) insights.push({cls:'good',title:'Dosing history active',text:history.length+' dose/check log'+(history.length===1?'':'s')+' saved for this tank.'});
      else insights.push({cls:'warn',title:'No dose checks logged yet',text:'Use Log Check or Log Dose so AquoraX can judge consistency from real saved data.'});
      const ce=products.filter(p=>p.ceProductId||metaFor(p)).length;
      insights.push({cls:'good',title:'Coral Essentials ecosystem active',text:ce+' Coral Essentials product'+(ce===1?'':'s')+' currently linked to this tank.'});
    }
    try{
      const signals=(typeof getLatestIcpSignals==='function'?getLatestIcpSignals():[])||[];
      if(signals.length) insights.push({cls:'warn',title:'ICP adjustment signals',text:`Latest ICP shows ${signals.length} non-OK item${signals.length===1?'':'s'}. Review chemistry before changing doses.`});
      else insights.push({cls:'warn',title:'No ICP signal yet',text:'Add ICP results to unlock chemistry-aware dosing guidance.'});
    }catch(e){}
  ...
  window.renderDosingPage=function(){
    const out=typeof oldRender==='function'?oldRender.apply(this,arguments):undefined;
    renderCeLibrary(); renderOverview(); renderInsights();
    return out;
  };
  document.addEventListener('DOMContentLoaded',function(){setTimeout(function(){try{renderCeLibrary();renderOverview();renderInsights();}catch(e){}},1200);});
})();
