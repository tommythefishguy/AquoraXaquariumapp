...
const aquoraxLockedNavItems = ["home","growth","jobs","dosing"];
...
function aqxApplyCleanNavDefaults(){
  ...
  ["parameters","history","graphs","guides","jobs","log"].forEach(id => prefs[id] = false);
  prefs.dosing = true;
  saveNavPrefs(prefs);
...
(function(){
  ...
  function renderTankOverview(){
    ...
    host.innerHTML=`
      <div class="aqxTankTopline">
        <div><h2>My Tanks</h2><p>See every tank at a glance. Tap a card to switch the active tank.</p></div>
      ...
      <div class="aqxTankOverviewGrid">
        ${tanks.map(t=>{const latest=latestTestFor(t.id); const st=tankStatus(latest); return `
          <button type="button" class="aqxTankCard ${t.id===activeId?'active':''}" onclick="aqxSwitchTank('${esc(t.id)}')">
            <div class="aqxTankCardHead"><strong>${esc(t.name)}</strong><span class="${st.cls}">${st.txt}</span></div>
            <small>${typeLabel(t.type)}${t.volume?' · '+esc(t.volume)+' L':''}</small>
            <div class="aqxTankParams">${parameterLine(latest,t.type)}</div>
            <em>${latest?'Last test: '+esc(latest.date||'Saved'):'Tap to start logging this tank'}</em>
          </button>`;}).join('')}
      </div>`;
  }
  function injectUi(){
  ...
  window.getParameterTests=function(){
    ...
    return typeof oldGetTests==='function'?oldGetTests():[];
  };
  window.saveParameterTests=function(tests){localStorage.setItem(scopedKey(SCOPED_KEYS.parameterTests), JSON.stringify(tests||[]));};
  // Jobs are scoped too, but legacy functions keep their names. Mirror active tank jobs into legacy keys before job screens render.
  function mirrorScopedToLegacy(){Object.values(SCOPED_KEYS).forEach(base=>{const v=localStorage.getItem(scopedKey(base)); if(v!=null) localStorage.setItem(base,v);});}
...
// === AquoraX Livestock Tracker Patch ===
(function(){
 function aqxActiveTankId(){return localStorage.getItem('aqx_active_tank')||'default';}
 function livestockKey(){return 'aqx_livestock_'+aqxActiveTankId();}
 function getLivestock(){try{return JSON.parse(localStorage.getItem(livestockKey())||'[]')}catch(e){return []}}
 function saveLivestock(v){localStorage.setItem(livestockKey(),JSON.stringify(v||[]));}
 function renderLivestock(){
   const home=document.getElementById('home'); if(!home) return;
   let card=document.getElementById('aqxLivestockCard');
   if(!card){
     card=document.createElement('div');
     card.id='aqxLivestockCard';
     card.style.cssText='margin:16px 0;padding:16px;border-radius:22px;background:rgba(255,255,255,0.06);backdrop-filter:blur(12px);color:white;';
     home.prepend(card);
   }
   const list=getLivestock();
   const fish=list.filter(x=>x.category==='Fish').length;
   const invert=list.filter(x=>x.category==='Invert').length;
   card.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;"><div><div style="font-size:22px;font-weight:700;">Livestock</div><div style="opacity:.7;font-size:13px;">Per tank fish & invert tracking</div></div><button onclick="aqxAddLivestock()" style="background:#2d8cff;border:none;color:white;padding:10px 14px;border-radius:14px;font-weight:700;">+ Add</button></div>
   <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;">
   <div style="padding:12px;border-radius:16px;background:rgba(0,0,0,0.25)"><div style="font-size:13px;opacity:.7">Fish</div><div style="font-size:28px;font-weight:700">${fish}</div></div>
   <div style="padding:12px;border-radius:16px;background:rgba(0,0,0,0.25)"><div style="font-size:13px;opacity:.7">Inverts</div><div style="font-size:28px;font-weight:700">${invert}</div></div>
   </div>
   <div>${list.length?list.map((x,i)=>`<div style="padding:10px;margin-bottom:8px;border-radius:14px;background:rgba(0,0,0,0.2);display:flex;justify-content:space-between;align-items:center;"><div><div style="font-weight:700">${x.name}</div><div style="font-size:12px;opacity:.7">${x.category}</div></div><button onclick="aqxDeleteLivestock(${i})" style="background:#ff4d4d;border:none;color:white;padding:8px 10px;border-radius:10px;">Remove</button></div>`).join(''):'<div style="opacity:.7">No livestock added yet.</div>'}</div>`;
 }
 window.aqxAddLivestock=function(){
   const name=prompt('Livestock name'); if(!name) return;
   let category=prompt('Type Fish or Invert','Fish')||'Fish';
   category=category.toLowerCase().includes('invert')?'Invert':'Fish';
   const arr=getLivestock(); arr.unshift({name,category,created:new Date().toISOString()}); saveLivestock(arr); renderLivestock();
 }
 window.aqxDeleteLivestock=function(i){const arr=getLivestock(); arr.splice(i,1); saveLivestock(arr); renderLivestock();}
 document.addEventListener('DOMContentLoaded',()=>setTimeout(renderLivestock,1200));
 setTimeout(renderLivestock,2000);
})();
