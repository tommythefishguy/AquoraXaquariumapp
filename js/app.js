...
function aqxHandleIcpFile(file){
  ...
  aqxIcpSetStatus('File ready: ' + file.name + '. Tap Scan Screenshot to auto-read it.');
  if(preview){
...
async function aqxStartIcpOcr(){
  ...
  try{
    ...
    const result = await T.recognize(aqxPendingIcpFile, 'eng', { logger:m => { if(m && m.status){ aqxIcpSetStatus('Scanning: ' + m.status + (m.progress ? ' ' + Math.round(m.progress*100) + '%' : '')); } } });
    const text = (result && result.data && result.data.text) || '';
    setVal('aqxIcpImportText', text.trim());
    aqxIcpSetStatus(text.trim() ? 'Scan complete. Values detected below — review before saving.' : 'Scan finished, but no clear text was detected. Try a sharper screenshot or paste report text.', text.trim() ? 'ok' : 'warn');
    aqxParseIcpText(text);
  }catch(err){
    console.error(err);
...
function aqxDetectIcpLab(text){
  ...
  if(t.includes('triton')) return 'Triton';
  if(t.includes('fauna marin') || t.includes('reef icp')) return 'Fauna Marin';
  if(t.includes('ati')) return 'ATI';
  if(t.includes('coral essentials')) return 'Coral Essentials';
  return val('icpLab') || 'ICP Import';
}
...
function aqxParseIcpText(text){
  text = String(text||'').trim();
  if(!text){ aqxIcpSetStatus('Paste ICP text or upload a screenshot first.', 'warn'); return; }
  const found = new Map();
  ...
  Object.keys(AQX_ICP_PARAM_ALIASES).forEach(key => {
    ...
    for(const line of all){
      for(const a of aliases){
        ...
        const re = new RegExp('(?:^|\\b)'+safe+'(?:\\b|\\s|:|=|-)\\s*[:=\\-]?\\s*(<|>)?\\s*(-?\\d+(?:[.,]\\d+)?)\\s*(ppm|ppt|mg\\/l|ug\\/l|µg\\/L|μg\\/L|dKH)?','i');
  ...
  if(!params.length){ aqxIcpSetStatus('No reef parameters detected. Try pasting clearer text with names and values.', 'warn'); return; }
  setVal('icpLab', aqxDetectIcpLab(text));
  setVal('icpDate', aqxDetectIcpDate(text));
  setVal('icpNotes', `Imported from ${aqxDetectIcpLab(text)} screenshot/text. Review values before saving.`);
  const box = el('icpParameterBuilder');
  if(box) box.innerHTML = '';
  params.forEach(p => addIcpParameterRow(p.name, p.value, p.unit, p.status));
  aqxRenderIcpImportReview(params);
  aqxIcpSetStatus(`Imported ${params.length} ICP parameters. Review the form, then tap Save ICP Test.`, 'ok');
  const form = el('icpParameterBuilder');
  if(form) form.scrollIntoView({behavior:'smooth', block:'center'});
}
function aqxRenderIcpImportReview(params){
  const box = el('aqxIcpImportReview');
  if(!box) return;
  box.style.display = 'grid';
  box.innerHTML = `<strong>Detected ${params.length} parameters</strong><div>${params.slice(0,18).map(p => `<span class="${p.status}">${p.name}: ${p.value} ${p.unit || ''} · ${String(p.status).toUpperCase()}</span>`).join('')}</div><small>Always review OCR results before saving. Screenshot quality can affect readings.</small>`;
}
