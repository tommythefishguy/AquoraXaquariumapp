...
const AQX_FIREBASE_BUILD = "fcm-real-time-20260507a";
...
function aqxSetSessionFromFirebase(user){
  if(!user) return;
...
(function(){
  ...
  async function swReg(){
    ...
    try{return await navigator.serviceWorker.register('./sw.js');}catch(e){console.warn('AquoraX service worker failed',e); return null;}
  }
...
(function(){
  ...
  const ENABLED_KEY='aqxNotificationsEnabledV1';
  const BUILD='fcm-real-time-20260507a';
  function $(id){return document.getElementById(id);}
  ...
  async function getMainServiceWorker(){
    if(!('serviceWorker' in navigator)) return null;
    try{
      const reg=await navigator.serviceWorker.register('./sw.js?v='+BUILD,{scope:'./'});
  ...
  async function registerFcmToken(){
    ...
    const key=vapidKey();
    if(!key){msg('Paste your Firebase Web Push certificate public key first, then tap Save Push Key.'); return null;}
  ...
  window.aqxSaveVapidKeyFromSettings=function(){
    const inp=$('aqxVapidKeyInput');
    const key=(inp && inp.value ? inp.value : '').trim();
    if(!key){msg('Missing Firebase push configuration.'); return;}
    localStorage.setItem(VAPID_KEY_STORAGE,key);
    msg('Firebase push configured. Tap Enable Real-Time Notifications.');
  };
  const oldEnable=window.aqxEnableNotifications;
  window.aqxEnableNotifications=async function(){
    fillKeyInput();
    if($('aqxVapidKeyInput') && $('aqxVapidKeyInput').value.trim()) localStorage.setItem(VAPID_KEY_STORAGE,$('aqxVapidKeyInput').value.trim());
    ...
    try{await registerFcmToken();}catch(e){console.warn('AquoraX FCM token failed',e); msg((e&&e.message)?e.message:'FCM token failed. Check the Web Push certificate key.');}
  };
  ...
  function updateCard(){
    ...
    else if(Notification.permission==='granted' && !key) msg('Notifications are allowed. Paste the Firebase Web Push certificate public key to activate real-time FCM.');
    else if(Notification.permission==='denied') msg('Notifications are blocked in browser/app settings.');
    else msg('Ready to enable. Paste the Firebase Web Push certificate public key, then tap Enable.');
  }
