...
function aqxClearIcpImportResults(opts={}){
  const {clearText=false, clearForm=false} = opts;
...
function aqxParseIcpText(text, opts={}){
  ...
  if(!text){ aqxClearIcpImportResults({clearForm: source === 'scan'}); aqxIcpSetStatus('Paste ICP text or upload a screenshot first.', 'warn'); return; }
  const found = new Map();
  ...
  if(!params.length){
    aqxClearIcpImportResults({clearForm: source === 'scan'});
