...
const aquoraxLockedNavItems = ["home","growth","care","cycle","profile"];
...
// === AquoraX Coral Essentials Dosing Ecosystem Patch ===
(function(){
  const CE_SOURCE_URL = 'https://finestaquatics.co.uk/search?q=coral+essentials';
  const CE_PRODUCTS = [
    {name:'Coral Power Amino', range:'Feed Range', purpose:'Amino Acid Complex', size:50, amount:2, unit:'ml', color:'#7de35f', code:'AM', note:'Concentrated amino acids for enzyme production, tissue growth and feeding response.'},
    {name:'Coral Power Gro', range:'Feed Range', purpose:'Growth Formula', size:100, amount:2, unit:'ml', color:'#8f62ff', code:'GR', note:'Vitamin complex including L-Lysine for growth, colouration and polyp extension.'},
    {name:'CVE+', range:'Black Label', purpose:'Colour + Vitality Enhancer', size:500, amount:4, unit:'ml', color:'#1aa7ff', code:'CVE', note:'All-in-one blend based around Chroma+, Vibrance+ and Energy+.'},
    {name:'Energy+', range:'Black Label', purpose:'Coral Energy', size:50, amount:1, unit:'ml', color:'#ff7a1a', code:'EN', note:'Black Label energy support for feeding and uptake.'},
    {name:'Vibrance+', range:'Black Label', purpose:'Colour Enhancement', size:50, amount:1, unit:'ml', color:'#e13288', code:'VI', note:'Colour and cell division support for reef vitality.'},
    {name:'Chroma+', range:'Black Label', purpose:'Colour & Pop', size:50, amount:1, unit:'ml', color:'#ffd21a', code:'CH', note:'Colour intensity support and metabolic coenzyme blend.'},
    {name:'Coral Power Trace A', range:'Trace Range', purpose:'Strontium + Barium', size:100, amount:1, unit:'ml', color:'#7857ff', code:'TA', note:'Trace-A blend for strontium and barium correction.'},
    {name:'Coral Power Trace C', range:'Trace Range', purpose:'Iodine + Fluorine', size:100, amount:1, unit:'ml', color:'#c64dff', code:'TC', note:'Trace-C blend for iodine and fluorine support.'},
    {name:'Coral Power Iodine', range:'Trace Range', purpose:'Iodine Support', size:50, amount:1, unit:'ml', color:'#00c7d8', code:'I', note:'Potassium iodide support for marine aquariums.'},
    {name:'Coral Power Phosphate Up', range:'Nutrient Control', purpose:'Phosphate Support', size:100, amount:1, unit:'ml', color:'#1da1ff', code:'PO4', note:'Balanced inorganic phosphorous source for low phosphate systems.'},
    {name:'Coral Power Bacteria Food', range:'Bacteria Range', purpose:'Bacteria Food', size:50, amount:1, unit:'ml', color:'#41dba2', code:'BF', note:'Acetic acids and sucrose blend to support bacterial systems.'},
    {name:'Coral Power Food', range:'Feed Range', purpose:'Powder Coral Food', size:50, amount:.5, unit:'g', color:'#d4a55a', code:'FD', note:'Micro powder coral food blend with plankton, mysis, rotifers and more.'}
  ];
  function $(id){return document.getElementById(id)}
  function esc(v){return String(v==null?'':v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}
  function activeTank(){try{return (window.aqxGetActiveTank&&window.aqxGetActiveTank())||{id:'tank_main',name:'Main Tank',type:'reef'};}catch(e){return {id:'tank_main',name:'Main Tank',type:'reef'};}}
  function scoped(base,id){try{return window.aqxScopedKey?window.aqxScopedKey(base,id||activeTank().id):(base+'__'+(id||activeTank().id));}catch(e){return base+'__tank_main';}}
  function read(base){try{const v=JSON.parse(localStorage.getItem(scoped(base))||'[]');return Array.isArray(v)?v:[];}catch(e){return [];}}
  function write(base,items){try{localStorage.setItem(scoped(base),JSON.stringify(items||[])); if(window.aqxQueueCloudBackup) window.aqxQueueCloudBackup('dosing');}catch(e){}}
  function num(v,f){const n=parseFloat(v);return Number.isFinite(n)?n:f;}
  function intervalDays(p){if(typeof doseIntervalDays==='function')return doseIntervalDays(p.frequency,p.interval);return p.frequency==='weekly'?7:(p.frequency==='custom'?Math.max(1,parseInt(p.interval||1,10)):1)}
  function daily(p){const a=num(p.amount,0), i=Math.max(1,num(p.interval||intervalDays(p),1));return a/i;}
  function daysLeft(p){const d=daily(p); if(d<=0)return null; return Math.floor(num(p.remainingAmount,p.bottleSize||0)/d);}
  function pctLeft(p){const size=num(p.bottleSize,0);if(size<=0)return 0;return Math.max(0,Math.min(100,(num(p.remainingAmount,size)/size)*100));}
  function ceForName(name){const n=String(name||'').toLowerCase();return CE_PRODUCTS.find(p=>n.includes(p.name.toLowerCase())||p.name.toLowerCase().includes(n))||null;}
  function stateFor(p){const left=daysLeft(p), low=num(p.lowStockDays,14); if(left===null)return {cls:'watch',txt:'No Data'}; if(left<=3)return {cls:'bad',txt:'Refill Soon'}; if(left<=low)return {cls:'warn',txt:'Low Stock'}; return {cls:'good',txt:'On Track'};}
  window.aqxSetCeMethod=function(method){localStorage.setItem(scoped('aquoraxCeMethod'),method||'sps'); document.querySelectorAll('.aqxCeMethodPill').forEach(b=>b.classList.remove('active')); const labels={sps:'SPS Dominant',mixed:'Mixed Reef',lps:'LPS / Soft Coral'}; document.querySelectorAll('.aqxCeMethodPill').forEach(b=>{if(b.textContent.trim()===labels[method])b.classList.add('active')}); renderCeOverview();};
  function currentMethod(){return localStorage.getItem(scoped('aquoraxCeMethod'))||'sps';}
  window.aqxFillCeProduct=function(index){const p=CE_PRODUCTS[index]; if(!p)return; const ids={doseProductName:p.name,doseProductMode:'auto',doseProductAmount:p.amount,doseProductUnit:p.unit,doseProductFrequency:'daily',doseProductBottleSize:p.size,doseProductRemaining:p.size,doseProductLowDays:'14',doseProductNotes:'Coral Essentials · '+p.range+' · '+p.purpose+'. '+p.note}; Object.keys(ids).forEach(id=>{const el=$(id); if(el) el.value=ids[id];}); try{toggleCustomDoseInterval();}catch(e){} const name=$('doseProductName'); if(name) name.scrollIntoView({behavior:'smooth',block:'center'});};
  window.aqxImportCeStarter=function(){const existing=read('aquoraxDosingProducts'); const names=new Set(existing.map(x=>String(x.name||'').toLowerCase())); const method=currentMethod(); const starter=(method==='lps'?['Coral Power Amino','Coral Power Gro','Coral Power Food','CVE+']:method==='mixed'?['Coral Power Amino','Coral Power Gro','CVE+','Coral Power Trace A','Coral Power Trace C']:['Coral Power Amino','Coral Power Gro','CVE+','Energy+','Vibrance+','Chroma+']); const add=CE_PRODUCTS.filter(p=>starter.includes(p.name)&&!names.has(p.name.toLowerCase())).map(p=>({id:Date.now()+Math.random().toString(36).slice(2,7),name:p.name,amount:p.amount,unit:p.unit,frequency:'daily',interval:1,mode:'auto',bottleSize:p.size,remainingAmount:p.size,lowStockDays:14,stockUpdatedIso:new Date().toISOString(),notes:'Coral Essentials · '+p.range+' · '+p.purpose+'. '+p.note,createdIso:new Date().toISOString(),lastDosedIso:null,ce:true,ceRange:p.range,cePurpose:p.purpose,ceColor:p.color,ceCode:p.code})); if(!add.length){alert('These Coral Essentials starter products are already added for this tank.');return;} write('aquoraxDosingProducts',existing.concat(add)); if(window.renderDosingPage) renderDosingPage();};
  function productPickerHtml(){return `<div id="aqxCeProductPicker" class="card aqxCeProductPicker"><div class="aqxCeSectionHead"><div><h2>Coral Essentials Product Library</h2><p>Actual Coral Essentials products/ranges stocked at Finest Aquatics. Tap one to preload the dosing form.</p></div><button class="secondaryBtn" onclick="aqxImportCeStarter()">Import Method Starter</button></div><div class="aqxCeProductRail">${CE_PRODUCTS.map((p,i)=>`<div class="aqxCePickCard" style="--ce:${esc(p.color)}"><div class="aqxCeBottle" data-code="${esc(p.code)}"></div><strong>${esc(p.name)}</strong><span>${esc(p.purpose)}</span><small>${esc(p.range)} · ${esc(p.size)}${esc(p.unit==='g'?'g':'ml')}</small><button type="button" onclick="aqxFillCeProduct(${i})">Use Product</button></div>`).join('')}</div><div class="aqxCeRanges"><div class="aqxCeRangeCard"><strong>Feed Range</strong><span>Amino, Gro, Coral Power Food and nutrient support.</span></div><div class="aqxCeRangeCard"><strong>Black Label</strong><span>CVE+, Energy+, Vibrance+ and Chroma+ ecosystem.</span></div><div class="aqxCeRangeCard"><strong>Trace Range</strong><span>Trace A/C, iodine, bromide, potassium and ICP-style corrections.</span></div><div class="aqxCeRangeCard"><strong>Bacteria / Nutrient</strong><span>Bacteria Food, Phosphate Up and supporting care products.</span></div></div><p class="smallPrint">Product data is saved locally per tank. This is not a shop page.</p></div>`}
  function ensureCePicker(){const dosing=$('dosing'); if(!dosing||$('aqxCeProductPicker'))return; const addCard=Array.from(dosing.querySelectorAll('.card')).find(c=>c.querySelector('#doseProductName')); if(addCard) addCard.insertAdjacentHTML('beforebegin',productPickerHtml()); const h=addCard&&addCard.querySelector('h2'); if(h) h.textContent='Add / Edit Dosing Product'; const schedule=Array.from(dosing.querySelectorAll('.card h2')).find(h=>h.textContent.trim()==='My Dosing Schedule'); if(schedule) schedule.textContent='Coral Essentials Products';}
  function renderCeOverview(){const box=$('aqxCeOverviewRings'); if(!box)return; const products=read('aquoraxDosingProducts'); const history=read('aquoraxDosingHistory'); const ceCount=products.filter(p=>ceForName(p.name)||p.ce).length; const lows=products.filter(p=>{const s=stateFor(p);return s.cls==='warn'||s.cls==='bad'}).length; const due=products.filter(p=>{try{return nextDueText(p).cls==='due'}catch(e){return false}}).length; const avgPct=products.length?Math.round(products.reduce((a,p)=>a+pctLeft(p),0)/products.length):0; const health=!products.length?0:Math.max(0,Math.min(100,Math.round(100-(lows*18)-(due*14)))); const consistency=!history.length?0:Math.min(100,55+Math.min(40,history.length*4)); const rings=[{cls:products.length?'good':'nodata',pct:health,val:products.length?health+'%':'—',label:'Dosing Health',sub:products.length?(health>=80?'Excellent':health>=60?'Tracking':'Needs review'):'No data yet'},{cls:history.length?'good':'nodata',pct:consistency,val:history.length?consistency+'%':'—',label:'Consistency',sub:history.length+' logs saved'},{cls:due?'warn':products.length?'good':'nodata',pct:products.length?Math.max(15,100-due*25):0,val:String(due),label:'Products Due',sub:due?'Review schedule':'None due'},{cls:lows?'bad':products.length?'watch':'nodata',pct:products.length?Math.max(8,100-lows*26):0,val:String(lows),label:'Low Stock',sub:lows?'Check inventory':(products.length?'Stock stable':'No products')},{cls:ceCount?'watch':'nodata',pct:ceCount?Math.min(100,ceCount*16):0,val:String(ceCount),label:'CE Products',sub:'Coral Essentials'}]; box.innerHTML=rings.map(r=>`<div class="aqxCeRing ${r.cls}" style="--pct:${r.pct}"><strong>${esc(r.val)}</strong><span>${esc(r.label)}</span><small>${esc(r.sub)}</small></div>`).join(''); const method=currentMethod(); document.querySelectorAll('.aqxCeMethodPill').forEach(b=>{b.classList.toggle('active',(method==='sps'&&b.textContent.includes('SPS'))||(method==='mixed'&&b.textContent.includes('Mixed'))||(method==='lps'&&b.textContent.includes('LPS')))});}
  function renderCeProducts(){const box=$('dosingProductList'); if(!box)return false; const items=read('aquoraxDosingProducts'); if(!items.length){box.innerHTML='<div class="aqxCeEmpty"><p class="smallPrint">No dosing products saved for this tank yet.</p><button class="primaryBtn" onclick="aqxImportCeStarter()">Import Coral Essentials Starter Method</button></div>';return true;} box.innerHTML=items.map(p=>{const ce=ceForName(p.name)||{}; const st=stateFor(p); const pct=pctLeft(p); const due=typeof nextDueText==='function'?nextDueText(p):{text:'Ready'}; const color=p.ceColor||ce.color||'#1aa7ff'; const code=p.ceCode||ce.code||(p.name||'?').slice(0,2).toUpperCase(); const unit=p.unit||'ml'; const dailyUse=daily(p); return `<div class="aqxCeDoseCard" style="--ce:${esc(color)}"><div class="aqxCeBottle" data-code="${esc(code)}" style="--ce:${esc(color)}"></div><div class="aqxCeDoseMain"><strong>${esc(p.name)}</strong><span>${esc(ce.purpose||p.notes||((p.mode||'manual')==='auto'?'Auto dosing':'Manual dosing'))}</span><span>${esc((p.mode||'manual')==='auto'?'Auto countdown':'Manual tap-to-dose')} · ${esc(activeTank().name||'Tank')}</span></div><div class="aqxCeDoseStats"><div class="aqxCePercent" style="--pct:${pct.toFixed(1)};--ring:${esc(st.cls==='bad'?'#ff3838':st.cls==='warn'?'#ff9f1a':'#52ff5e')}"><b>${Math.round(pct)}%</b></div><div class="aqxCeDoseMini"><strong>${dailyUse?dailyUse.toFixed(dailyUse<10?1:0):'—'} ${esc(unit)}/day</strong><span>${esc(due.text||'Ready')}</span></div><div class="aqxCeState ${st.cls}">${esc(st.txt)}</div></div><div class="aqxCeCardActions"><button class="primaryBtn" onclick="logDose('${esc(p.id)}')">${(p.mode||'manual')==='auto'?'Log Check':'Log Dose'}</button><button class="secondaryBtn" onclick="aqxRefillDosingProduct&&aqxRefillDosingProduct('${esc(p.id)}')">Refill</button><button class="dangerBtn" onclick="deleteDosingProduct('${esc(p.id)}')">Remove</button></div></div>`}).join(''); return true;}
  function renderCeInsights(){const box=$('dosingInsightList'); if(!box)return; const products=read('aquoraxDosingProducts'); const history=read('aquoraxDosingHistory'); const lows=products.filter(p=>['warn','bad'].includes(stateFor(p).cls)); const ceProducts=products.filter(p=>ceForName(p.name)||p.ce); const insights=[]; if(!products.length) insights.push({cls:'warn',title:'No Coral Essentials products added yet',text:'Import a Coral Essentials starter method or add individual products to unlock dosing intelligence.'}); else insights.push({cls:'good',title:'Coral Essentials ecosystem active',text:`${ceProducts.length} Coral Essentials product${ceProducts.length===1?'':'s'} tracked for ${activeTank().name||'this tank'}.`}); if(lows.length) insights.push({cls:lows.some(p=>stateFor(p).cls==='bad')?'bad':'warn',title:'Inventory attention required',text:`${lows.map(p=>p.name).slice(0,3).join(', ')} ${lows.length===1?'is':'are'} approaching refill based on saved stock and dose rate.`}); else if(products.length) insights.push({cls:'good',title:'Stock countdowns look controlled',text:'No saved dosing products are currently inside their low-stock warning window.'}); if(history.length>=3) insights.push({cls:'good',title:'Consumption pattern building',text:`${history.length} dosing logs are saved. AquoraX can now start showing consistency and refill behaviour.`}); else insights.push({cls:'warn',title:'More logs needed',text:'Log checks or manual doses over the next few days to improve consumption predictions.'}); try{const signals=aqxLatestIcpSignals?aqxLatestIcpSignals():[]; if(signals.length) insights.push({cls:'warn',title:'ICP signals detected',text:`Latest ICP has ${signals.length} non-OK marker${signals.length===1?'':'s'}: ${signals.map(s=>s.name+' '+s.status).slice(0,3).join(', ')}.`}); else insights.push({cls:'good',title:'ICP signal status',text:'No low/high/watch markers found in the latest saved ICP test.'});}catch(e){} box.innerHTML=insights.map(i=>`<div class="dosingInsightItem ${i.cls}"><strong>${esc(i.title)}</strong><span>${esc(i.text)}</span></div>`).join('');}
  const oldRender=window.renderDosingPage;
  window.renderDosingPage=function(){ensureCePicker(); let out; try{out=oldRender&&oldRender.apply(this,arguments);}catch(e){console.warn('AquoraX dosing render fallback',e)} ensureCePicker(); renderCeOverview(); renderCeProducts(); renderCeInsights(); return out;};
  const oldProducts=window.renderDosingProducts; window.renderDosingProducts=function(){if(!renderCeProducts()&&oldProducts)return oldProducts.apply(this,arguments);};
  const oldInsights=window.renderDosingInsights; window.renderDosingInsights=function(){try{if(oldInsights)oldInsights.apply(this,arguments);}catch(e){} renderCeInsights();};
  document.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{try{window.renderDosingPage&&window.renderDosingPage()}catch(e){}},1200));
})();
