...
(function(){
  ...
  function buildDashboard(){
    ...
    const health=[
      {label:'Dosing',value:lowDosing.length?lowDosing.length+' low':'Stable',cls:lowDosing.length?'warn':'good'},
      {label:'Equipment',value:equipDue.length?equipDue.length+' due':'Good',cls:equipDue.some(e=>daysUntil(e.nextService)<0)?'bad':(equipDue.length?'warn':'good')},
      {label:'Jobs',value:jobsAttention.length?jobsAttention.length+' due':'Clear',cls:jobsAttention.some(j=>jobStatus(j)==='overdue')?'bad':(jobsAttention.length?'warn':'good')},
      {label:'Testing',value:age===null?'Needed':(age>=7?'Due':age+'d ago'),cls:age===null||age>=7?'warn':'good'}
