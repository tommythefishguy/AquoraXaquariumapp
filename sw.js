const AQX_CACHE = 'aquorax-pwa-install-fixed-20260507d';
const AQX_ASSETS = ['./','./index.html','./manifest.webmanifest','./css/styles.css?v=pwa-install-fixed-20260507d','./js/app.js?v=pwa-install-fixed-20260507d','./aquorax-logo.png','./assets/icon-192.png','./assets/icon-512.png','./assets/icon-maskable-192.png','./assets/icon-maskable-512.png'];
...
self.addEventListener('fetch', event => {
  ...
  const isFreshAsset = url.pathname.endsWith('/index.html') || url.pathname.endsWith('/js/app.js') || url.pathname.endsWith('/css/styles.css') || url.pathname.endsWith('/manifest.webmanifest') || url.pathname === '/' || url.pathname.endsWith('/');
  if(isFreshAsset){
...
self.addEventListener('notificationclick', event => {
  ...
  const target = (event.notification && event.notification.data && event.notification.data.url) || './index.html?v=pwa-install-fixed-20260507d';
